...
-- <시스템 계정으로 실행>
GRANT CREATE SESSION TO C##SAMPLE; -- 부여하다. CREATE CESSION. SAMPLE에.
...
WITH ADMIN OPTION;  -- 접속 권한 + 접속 권한을 다른 사용자에게 줄 수 있는 권한도 부여
-- <시스템 계정으로 실행>
...
SELECT
       EE.*
FROM C##EMPLOYEE.DEPARTMENT ED; -- DEPARTMENT의 권한은 부여받지 못했기 때문에 인식하지 못한다.

-- <샘플 계정으로 실행>
-- WITH GRANT OPTION으로 부여 받은 SELECT 권한은 다른 계정에게 부여 가능하나
-- 그 외의 INSERT와 같은 권한은 부여할 수 없다.
GRANT SELECT ON C##EMPLOYEE.EMPLOYEE TO C##SAMPLE2; -- 샘플계정이 SAMPLE2에도 권한을 줄 수 있다.
GRANT INSERT ON C##EMPLOYEE.EMPLOYEE TO C##SAMPLE2; -- INSERT는 권한 갖고 있지 않아 불가능
...
-- WITH ADMIN OPTION은 특정 사용자의 권한만 회수 되고 나머지 사용자에게 부여한 권한은
-- 회수 되지 않는다.
