...
-- 노옹철 사원과 같은 부서에 소속 된 사원 조회 (조회하지 않으면 알 수 없음, 조회 먼저)

-- 사원명이 노옹철인 사람의 부서 조회 <- 서브쿼리
SELECT
       DEPT_CODE
  FROM EMPLOYEE
 WHERE EMP_NAME = '노옹철';
 
-- 부서코드가 D9인 직원 조회 <- 메인쿼리
SELECT
       EMP_NAME
  FROM EMPLOYEE
 WHERE DEPT_CODE = 'D9';
 
-- 위의 두 쿼리를 하나로 작성한다. 하나는 메인쿼리, 하나는 서브쿼리
SELECT
       EMP_NAME
  FROM EMPLOYEE
 WHERE DEPT_CODE = (SELECT
                          DEPT_CODE
                     FROM EMPLOYEE
                    WHERE EMP_NAME = '노옹철');    -- 커서 잘 씌워서 하면 서브쿼리, 메인쿼리만 따로 실행 가능
                    
-- 전 직원의 평균 급여(서브)보다 많은 급여를 받고 있는 직원의 사번, 이름, 직급 코드, 급여 조회(메인)
SELECT
       EMP_ID
     , EMP_NAME
     , JOB_CODE
...
-- 부서별 최고 급여와 동일 급여를 받는 직원의 이름, 부서, 급여 조회
SELECT
      EMP_NAME
    , DEPT_CODE
    , SALARY
 FROM EMPLOYEE
WHERE SALARY IN (SELECT MAX(SALARY) FROM EMPLOYEE GROUP BY DEPT_CODE); -- IN : 소괄호 안에 여러값들을 나열하는 연산자

-- 대리 직급의 직원들(메인) 중에서 과장 직급의 최소 급여(서브)보다 많이 받는 직원 조회
-- 사번, 이름, 직급명, 급여를 조회
--   > ANY / < ANY : 다중행 서브쿼리를 비교할 때 사용하는 연산자
--   > ANY : 서브 쿼리 결과의 최소 값 보다 큰 값을 비교
--   < ANY : 서브 쿼리 결과의 최대값 값 보다 작은 값을 비교
SELECT
      EMP_ID
    , EMP_NAME
    , JOB_NAME
...
WHERE JOB_NAME = '대리'
  AND SALARY > ANY (SELECT                    -- ANY 서브쿼리 최소 값보다 큰가?
                      SALARY
                 FROM EMPLOYEE
                 JOIN JOB USING(JOB_CODE)
                 WHERE JOB_NAME = '과장');

-- 차장 직급의 급여(서브)의 가장 큰 값보다 많이 받는 과장 직급(메인)의 사원 조회
--   > ALL : 서브쿼리의 결과 중 최대 값 보다 큰 값 비교
--   < ALL : 서브쿼리의 결과 중 최소 값 보다 작은 값 비교
SELECT
       EMP_ID
     , EMP_NAME
     , JOB_NAME
...
-- ROWNUM이 1로 부여가 되므로 모든 행은 6~10 사이라는 조건을 만족할 수 없어 결과가 0행이 된다.
SELECT
       ROWNUM
     , V.EMP_NAME
     , V.SALARY
...
SELECT
       EMP_NAME
     , SALARY
     , RANK() OVER(ORDER BY SALARY DESC) 순위         -- 19등 2명 - 21등
  FROM EMPLOYEE;
  
SELECT
       EMP_NAME
     , SALARY
     , DENSE_RANK() OVER(ORDER BY SALARY DESC) 순위   -- 19등 2명 - 20등
  FROM EMPLOYEE;
...
SELECT
 ...
 WHERE EXISTS (SELECT
                      E2.EMP_ID
                 FROM EMPLOYEE E2
                WHERE E.MANAGER_ID = E2.EMP_ID      -- 서브쿼리쪽에 메인쿼리의 값이 들어가 있다.
);
