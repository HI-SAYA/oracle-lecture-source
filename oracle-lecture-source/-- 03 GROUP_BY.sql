...
-- 부서별 급여 합계, 평균(정수처리), 인원 수 조회 수 부서 코드 순 오름차순 정렬
SELECT
       DEPT_CODE 부서
     , SUM(SALARY) 합계
     , ROUND(AVG(SALARY)) 평균
  ...
  FROM EMPLOYEE
 GROUP BY DEPT_CODE
 ORDER BY DEPT_CODE NULLS FIRST;          -- 항상 맨 끝에 적는다. / NULLS FIRST 맨 앞에 조회하고 싶은 경우
-- ORDER BY 부서;
-- ORDER BY 4 DESC, 1;     -- 인원수, 역순정렬 
...
SELECT
    ...
    , ROUND(AVG(SALARY)) 평균
 FROM EMPLOYEE
WHERE SALARY >= 3000000         -- 조건 먼저, 그룹바이 다음
GROUP BY DEPT_CODE
ORDER BY 1;
...
SELECT
 ...
 FROM EMPLOYEE
GROUP BY DEPT_CODE
HAVING ROUND(AVG(SALARY)) >= 3000000        -- 그룹바이에 대한 조건
ORDER BY 1;
...
FROM EMPLOYEE
GROUP BY CUBE(DEPT_CODE, JOB_CODE)              -- "직급별 그룹 묶인 상태" 1 <-아니다 라는 뜻
ORDER BY 1;                                     -- 묶여있으면 0
