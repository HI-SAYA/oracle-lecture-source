...
(
SELECT      -- VALUES 대신에 서브쿼리 사용해서 값 기재
EMP_ID
, EMP_NAME
, DEPT_TITLE
FROM EMPLOYEE
...
SELECT
    ...
    , HIRE_DATE
FROM EMPLOYEE
WHERE 1 = 0;            -- ALWAYS FALSE다. 값은 가져가고 싶지 않고, 구조만 따가고 싶을 경우 WHERE 절을 FALSE로 만든다.
...
INSERT ALL
    WHEN HIRE_DATE < '2000/01/01'       -- 2000/01/01 이전이면 EMP_OLD 테이블을 이용
    THEN
    INTO EMP_OLD
VALUES
(
...
WHEN HIRE_DATE >= '2000/01/01'          -- 2000/01/01 이후이면 EMP_NEW 테이블 이용
THEN
INTO EMP_NEW
VALUES
    (
...
-- ORA-02291: 무결성 제약조건(C##EMPLOYEE.SYS_C007449)이 위배되었습니다- 부모 키가 없습니다
UPDATE
        EMPLOYEE
    SET DEPT_CODE = '20'            -- 부모 키에 20이 없어서 수정이 안된다.
  WHERE DEPT_CODE = 'D6';
  
-- SQL 오류: ORA-01407: NULL로 ("C##EMPLOYEE"."EMPLOYEE"."EMP_NAME")을 업데이트할 수 없습니다 / NOT NULL 제약조건에 걸린다.
UPDATE
       EMPLOYEE
    SET EMP_NAME = NULL
    WHERE EMP_ID = '200';       -- WHERE 절을 이용해서 반드시 해야한다.
...
MERGE
INTO EMP_M01 M1
USING EMP_M02 M2        -- 2번 테이블을 이용해서 1번 테이블 쪽으로 합친다.
ON (M1.EMP_ID = M2.EMP_ID) -- 기준이 같으면
WHEN MATCHED THEN       -- 업데이트 할 것이다.
UPDATE                  -- 2번 테이블에 있는 컬럼들을 1번 테이블 쪽으로 옮겨줄 것.
...
-- 무결성 제약조건(C##EMPLOYEE.SYS_C007449)이 위배되었습니다- 자식 레코드가 발견되었습니다
DELETE
FROM DEPARTMENT
WHERE DEPT_ID = 'D1';
...
-- TRUNCATE : 테이블의 전체 행 삭제 시 사용 / 정확하게는 DDL 파트에 들어간다. - 자동으로 COMMIT되어버려서 수행하는 순간 바로 저장되어버림
