...
-- 시스템 계정으로 변환하여 뷰 객체 생선 권한을 부여한 뒤 작업한다.
...
-- 베이스테이블의 정보가 변경 되면 VIEW의 정보도 같이 변경된다.
SELECT
      V.*
FROM V_RESULT_EMP V
WHERE EMP_ID = '200';
...
SELECT
    ...
    , EMP_NAME
    , JOB_NAME
    , DECODE(SUBSTR(EMP_NO, 8, 1), 1, '남', '여')                 -- 함수식을 컬럼명화 불가능, 반드시 별칭이 필요하다.
...
-- VIEW를 이용한 DML 구문 동작 테스트 (VIEW 수정으로 베이스 테이블 수정 가능한가?)
...
INSERT
  INTO V_JOB        -- VIEW를 통한 INSERT 값이 베이스 테이블에도 들어가 있다.
(
    JOB_CODE
  , JOB_NAME
)
...
INSERT
    INTO V_JOB3
(
JOB_CODE            -- JOB_CODE는 부적합 하다. V_JOB3 뷰에 JOB_NAME만 있다.
, JOB_NAME
)
VALUES
(
...
UPDATE
        V_JOB3
    SET JOB_NAME = '인턴'         -- 사원을 찾아서 인턴으로 바꾸겠다.
  WHERE JOB_NAME = '사원';
...
, EMP_NAME 
, SALARY 
, (SALARY + (SALARY * NVL(BONUS, 0))) * 12 연봉       -- 별칭 꼭 붙여줘야함 (산술 표현식이라서)
FROM EMPLOYEE;
-- 가상 컬럼에는 INSERT 불가
INSERT
INTO EMP_SAL
...
, '정진훈' 
, 3000000 
, 40000000          -- ORA-01733: 가상 열은 사용할 수 없습니다 / "virtual column not allowed here"
);
...
-- 키-보존된것이 아닌 테이블로 대응한 열을 수정할 수 없습니다 -- 베이스 테이블에서 해라
UPDATE
      V_JOINEMP V
...
-- 가상 컬럼에 INSERT 불가 / 가상 열(컬럼)은 사용할 수 없습니다
INSERT
INTO V_GROUPDEPT
(
  DEPT_CODE
...
-- VIEW OPTION
-- FORCE / NOFORCE
CREATE OR REPLACE FORCE VIEW V_EMP   -- 기본적으로는 NOFOCE
AS
SELECT TCODE
    , TNAME
    , TCONTENT
...
UPDATE
        V_EMP2
    SET MANAGER_ID = '900'
  WHERE MANAGER_ID = '200'; -- 뷰의 WITH CHECK OPTION의 조건에 위배 됩니다
